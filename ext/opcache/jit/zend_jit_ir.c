...
/* Callee-saved registers sysv in ABI, and first two preserve_none args. */
...
# define ir_CAST_OPCODE_HANDLER_FUNC(_addr)  ir_fold2(_ir_CTX, IR_OPT(IR_PROTO, IR_ADDR), (_addr), \
       ir_proto_0(_ir_CTX, IR_OPCODE_HANDLER_FUNC, IR_ADDR))
#else
# define ir_CAST_OPCODE_HANDLER_FUNC(_addr) _addr
#endif
...
#define ir_RET2_A(ref) ({ (void)ref; ir_RLOAD(IR_ADDR, 2 /* IR_REG_RDX */); })
...
#define JIT_STUBS(_) \
	...
	_(tailcall_func_trace_counter,    IR_FUNCTION | IR_OPCODE_HANDLER_FUNC) \
	_(tailcall_ret_trace_counter,     IR_FUNCTION | IR_OPCODE_HANDLER_FUNC) \
	_(tailcall_loop_trace_counter,    IR_FUNCTION | IR_OPCODE_HANDLER_FUNC) \
...
static void zend_jit_tailcall_handler(zend_jit_ctx *jit)
{
	if (GCC_GLOBAL_REGS) {
		ir_TAILCALL(IR_VOID, ir_CAST_OPCODE_HANDLER_FUNC(ir_LOAD_A(jit_IP(jit))));
	} else {
		ir_ref ip = jit_IP(jit);
		ir_TAILCALL_2(IR_ADDR, ir_CAST_OPCODE_HANDLER_FUNC(ir_LOAD_A(ip)), jit_FP(jit), ip);
	}
}

static void zend_jit_tailcall_handler_ex(zend_jit_ctx *jit, ir_ref handler)
{
#if defined(IR_TARGET_X86) || ZEND_VM_TAIL_CALL_DISPATCH
...
static int zend_jit_exception_handler_stub(zend_jit_ctx *jit)
{
	...
	if (zend_jit_vm_kind == ZEND_VM_KIND_HYBRID) {
...
#if ZEND_VM_TAIL_CALL_DISPATCH
		ir_ref ip = ir_CALL(IR_ADDR, ir_CONST_OPCODE_HANDLER_FUNC(handler));
		ir_ref fp = ir_RET2_A(ip);
		ir_TAILCALL_2(IR_ADDR, ir_CAST_OPCODE_HANDLER_FUNC(ir_LOAD_A(ip)), fp, ip);
#else
		ir_CALL(IR_VOID, ir_CONST_OPCODE_HANDLER_FUNC(handler));
		ir_TAILCALL(IR_VOID, ir_CAST_OPCODE_HANDLER_FUNC(ir_LOAD_A(jit_IP(jit))));
#endif
	} else {
		...
		if (GCC_GLOBAL_REGS) {
			ir_TAILCALL(IR_VOID, ir_CONST_OPCODE_HANDLER_FUNC(handler));
		} else {
...
static int zend_jit_leave_function_handler_stub(zend_jit_ctx *jit)
{
	...
	if (ZEND_VM_TAIL_CALL_DISPATCH) {
		ir_ref ip = ir_CALL_3(IR_ADDR, ir_CONST_FC_FUNC(zend_jit_leave_nested_func_helper), jit_FP(jit), jit_IP(jit), call_info);
		ir_ref fp = ir_RET2_A(ip);
		ir_TAILCALL_2(IR_ADDR, ir_CAST_OPCODE_HANDLER_FUNC(ir_LOAD_A(ip)), fp, ip);
...
static int zend_jit_hybrid_runtime_jit_stub(zend_jit_ctx *jit)
{
	...
	if (ZEND_VM_TAIL_CALL_DISPATCH) {
		ir_ref ip = ir_CALL(IR_ADDR, ir_CONST_OPCODE_HANDLER_FUNC(zend_runtime_jit));
		ir_ref fp = ir_RET2_A(ip);
		ir_TAILCALL_2(IR_ADDR, ir_LOAD_A(ip), fp, ip);
	} else {
...
static int _zend_jit_trace_counter_stub(zend_jit_ctx *jit, uint32_t cost)
{
	...
	ir_IF_FALSE(if_overflow);
	zend_jit_tailcall_handler_ex(jit, _zend_jit_orig_opline_handler(jit, offset));

	ir_IF_TRUE(if_halt);
	zend_jit_tailcall_handler_ex(jit, ir_CONST_OPCODE_HANDLER_FUNC(zend_jit_halt_op->handler));
...
static int zend_jit_hybrid_func_trace_counter_stub(zend_jit_ctx *jit)
{
	...
	return _zend_jit_trace_counter_stub(jit,
		((ZEND_JIT_COUNTER_INIT + JIT_G(hot_func) - 1) / JIT_G(hot_func)));
}
...
/* Same stubs as hybrid, but compiled as functions */

static int zend_jit_tailcall_func_trace_counter_stub(zend_jit_ctx *jit)
{
	if (!ZEND_VM_TAIL_CALL_DISPATCH || !JIT_G(hot_func)) {
		return 0;
	}
...
static int zend_jit_tailcall_ret_trace_counter_stub(zend_jit_ctx *jit)
{
	if (!ZEND_VM_TAIL_CALL_DISPATCH || !JIT_G(hot_return)) {
		return 0;
	}
...
static int zend_jit_tailcall_loop_trace_counter_stub(zend_jit_ctx *jit)
{
	if (!ZEND_VM_TAIL_CALL_DISPATCH || !JIT_G(hot_loop)) {
		return 0;
	}
...
static int zend_jit_trace_exit_stub(zend_jit_ctx *jit)
{
	...
	if (GCC_GLOBAL_REGS || ZEND_VM_TAIL_CALL_DISPATCH) {
		zend_jit_tailcall_handler_ex(jit, addr);
	} else {
#if defined(IR_TARGET_X86)
...
static void zend_jit_init_ctx(zend_jit_ctx *jit, uint32_t flags)
{
	...
	if (!(flags & IR_FUNCTION)) {
		...
		if (zend_jit_vm_kind == ZEND_VM_KIND_CALL || ZEND_VM_TAIL_CALL_DISPATCH) {
			...
			if (GCC_GLOBAL_REGS) {
				...
				jit->ctx.fixed_regset |= (1<<IR_REG_FP) | (1<<3 /* IR_REG_RBX */) | (1<<10 /* IR_REG_R10 */);
			} else {
...
static int zend_jit_handler(zend_jit_ctx *jit, const zend_op *opline, int may_throw)
{
	...
	if (GCC_GLOBAL_REGS) {
		...
		if (ZEND_VM_TAIL_CALL_DISPATCH) {
			/* Storing FP is always required as it's not preserved */
			ir_ref fp = ir_RET2_A(ip);
			jit_STORE_FP(jit, fp);
		}
		jit_STORE_IP(jit, ip);
...
static int zend_jit_tail_handler(zend_jit_ctx *jit, const zend_op *opline)
{
	...
	zend_jit_set_ip(jit, opline);
	if (ZEND_VM_TAIL_CALL_DISPATCH) {
		zend_jit_tailcall_handler_ex(jit, ir_CONST_OPCODE_HANDLER_FUNC(opline->handler));
	} else if (zend_jit_vm_kind == ZEND_VM_KIND_HYBRID) {
		...
		if (trace && trace->op != ZEND_JIT_TRACE_END) {
			...
			if (may_be_top_frame) {
				...
				if (zend_jit_vm_kind == ZEND_VM_KIND_HYBRID) {
					...
					ir_GUARD(ir_NE(jit_IP(jit), ir_CONST_ADDR(ZEND_VM_ENTER_BIT)), jit_STUB_ADDR(jit, jit_stub_trace_halt));
				}
			}
...
#if defined(ZEND_VM_HYBRID_JIT_RED_ZONE_SIZE) || ZEND_VM_TAIL_CALL_DISPATCH
				zend_jit_tailcall_handler(jit);
#else
				ir_IJMP(jit_STUB_ADDR(jit, jit_stub_trace_escape));
#endif
...
static int zend_jit_trace_handler(zend_jit_ctx *jit, const zend_op_array *op_array, const zend_op *opline, int may_throw, zend_jit_trace_rec *trace)
{
	...
	if (GCC_GLOBAL_REGS) {
		...
		ref = ir_CALL_2(IR_ADDR, ir_CONST_OPCODE_HANDLER_FUNC(handler), jit_FP(jit), jit_IP(jit));
		/* Storing FP is always required as it's not preserved */
		fp = ir_RET2_A(ref);
		jit_STORE_FP(jit, fp);
		jit_STORE_IP(jit, ref);
...
static int zend_jit_trace_return(zend_jit_ctx *jit, bool original_handler, const zend_op *opline)
{
	if (GCC_GLOBAL_REGS || ZEND_VM_TAIL_CALL_DISPATCH) {
		if (!original_handler) {
			...
			zend_jit_tailcall_handler_ex(jit, zend_jit_orig_opline_handler(jit));
		}
	} else {
		if (original_handler) {
			ir_ref ref;
