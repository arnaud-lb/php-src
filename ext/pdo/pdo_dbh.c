...
/* We can not add Deprecation attributes directly in @generate-function-entries
 * stubs, and PDO drivers have no ways to add them either, so we hard-code
 * deprecation info here and add the attribute manually in pdo_hash_methods().
 */
